...
serve(async (req) => {
  ...
  try {
    ...
    if (!openaiKey) {
      ...
      return new Response(JSON.stringify({ error: 'AI features not configured', recap: null }), {
        status: 200,
    ...
    if (!memories || memories.length === 0) {
      ...
      return new Response(
        JSON.stringify({
          recap:
            "This journey doesn't have any memories captured yet. Every adventure starts somewhere!",
          highlights: [],
        }),
    ...
    if (primaryCondition) {
      ...
      if (sortedConditions.length > 1) {
        const otherConditions = sortedConditions
          .slice(1, 3)
          .map(([c]) => c.toLowerCase())
          .join(', ');
...
Respond in JSON format:
{
  ...
  "highlights": ["Highlight 1", "Highlight 2"]
        ...
        messages: [
          {
            role: 'system',
            content:
              'You are a warm, thoughtful travel memory assistant. Always respond in valid JSON.',
          },
    ...
    if (!content) {
      ...
      return new Response(JSON.stringify({ error: 'Failed to generate recap', recap: null }), {
        status: 200,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    try {
      ...
      console.log(
        '[generate-recap] Successfully parsed response, recap length:',
      ...
      console.warn(
        '[generate-recap] Failed to parse JSON response, using raw content:',
        parseError
      );
      return new Response(
        JSON.stringify({
    ...
    return new Response(JSON.stringify({ error: 'Internal server error', recap: null }), {
      status: 500,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  }
